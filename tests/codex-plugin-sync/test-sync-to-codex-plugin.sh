...
EOF
    ...
    cat > "$repo/assets/wukong-code-small.svg" <<'EOF'
...
write_destination_fixture() {
    ...
    mkdir -p "$repo/plugins/wukong-code/skills/example"
    printf 'fixture keep\n' > "$repo/plugins/wukong-code/.fixture-keep"
    cat > "$repo/plugins/wukong-code/skills/example/SKILL.md" <<'EOF'
# Example Skill

Fixture content.
EOF
    git -C "$repo" add plugins/wukong-code/.fixture-keep
    git -C "$repo" add plugins/wukong-code/skills/example/SKILL.md
...
add_openai_agent_metadata_fixture() {
    ...
    mkdir -p "$repo/plugins/wukong-code/skills/example/agents"

    cat > "$repo/plugins/wukong-code/skills/example/agents/openai.yaml" <<'EOF'
interface:
  display_name: "Example"
  short_description: "Destination-owned OpenAI metadata"
EOF

    git -C "$repo" add plugins/wukong-code/skills/example/agents/openai.yaml
...
write_synced_destination_fixture() {
    ...
    mkdir -p \
        "$repo/plugins/wukong-code/.codex-plugin" \
    ...
    cat > "$repo/plugins/wukong-code/.codex-plugin/plugin.json" <<EOF
{
  "name": "wukong-code",
  "version": "$MANIFEST_VERSION"
}
EOF
    ...
    cat > "$repo/plugins/wukong-code/assets/wukong-code-small.svg" <<'EOF'
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1 1"></svg>
EOF

    printf 'png fixture\n' > "$repo/plugins/wukong-code/assets/app-icon.png"

    cat > "$repo/plugins/wukong-code/hooks/hooks-codex.json" <<'EOF'
{
  "hooks": {
    "SessionStart": [
      {
...
EOF
    ...
    cat > "$repo/plugins/wukong-code/hooks/session-start" <<'EOF'
#!/usr/bin/env sh
echo "session-start fixture"
EOF
    cat > "$repo/plugins/wukong-code/hooks/session-start-codex" <<'EOF'
#!/usr/bin/env sh
echo "session-start-codex fixture"
EOF

    cat > "$repo/plugins/wukong-code/hooks/run-hook.cmd" <<'EOF'
@echo off
echo run-hook fixture
EOF
...
EOF
    ...
    printf 'tracked keep\n' > "$repo/plugins/wukong-code/.private-journal/keep.txt"

    git -C "$repo" add \
        plugins/wukong-code/.codex-plugin/plugin.json \
        plugins/wukong-code/assets/app-icon.png \
        plugins/wukong-code/assets/wukong-code-small.svg \
        plugins/wukong-code/hooks/hooks-codex.json \
...
write_stale_ignored_destination_fixture() {
    ...
    mkdir -p \
        "$repo/plugins/wukong-code/.kimi-plugin" \
        "$repo/plugins/wukong-code/.private-journal"
    printf 'fixture keep\n' > "$repo/plugins/wukong-code/.fixture-keep"
    printf '{"name":"stale-kimi"}\n' > "$repo/plugins/wukong-code/.kimi-plugin/plugin.json"
    printf 'stale ignored leak\n' > "$repo/plugins/wukong-code/.private-journal/leak.txt"
    git -C "$repo" add \
        plugins/wukong-code/.fixture-keep \
        plugins/wukong-code/.kimi-plugin/plugin.json
...
main() {
    ...
    dirty_skill_path="$dirty_apply_dest/plugins/wukong-code/skills/example/SKILL.md"
    noop_openai_metadata_path="$noop_apply_dest/plugins/wukong-code/skills/example/agents/openai.yaml"
    ...
    assert_not_contains "$preview_output" "Assets (wukong-code-small.svg, app-icon.png) will be seeded from" "Preview omits assets seeding note"
    ...
    assert_branch_absent "$dest" "sync/wukong-code-*" "Preview does not create sync branch in destination checkout"
    ...
    assert_contains "$bootstrap_output" "Mode:     BOOTSTRAP (creating plugins/wukong-code/ when absent)" "Bootstrap preview describes directory creation"
    ...
    assert_path_absent "$bootstrap_dest/plugins/wukong-code" "Bootstrap preview does not create destination plugin directory"
    ...
    assert_branch_absent "$bootstrap_dest" "bootstrap/wukong-code-*" "Bootstrap preview does not create bootstrap branch in destination checkout"

    echo ""
    echo "Apply assertions..."
    assert_equals "$dirty_apply_status" "1" "Dirty local apply exits with failure"
    assert_contains "$dirty_apply_output" "ERROR: local checkout has uncommitted changes under 'plugins/wukong-code'" "Dirty local apply reports protected destination path"
    ...
    assert_branch_absent "$dirty_apply_dest" "sync/wukong-code-*" "Dirty local apply does not create sync branch in destination checkout"
...
Locally modified fixture content." "Dirty local apply preserves tracked working-tree file content"
    ...
    assert_branch_absent "$noop_apply_dest" "sync/wukong-code-*" "Clean no-op local apply does not create sync branch in destination checkout"
