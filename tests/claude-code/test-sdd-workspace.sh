...
main() {
    ...
    if [[ "$dir" == "$repo/.wukong-code/sdd" ]]; then
        pass "prints <repo-root>/.wukong-code/sdd"
    else
        fail "prints <repo-root>/.wukong-code/sdd"
        echo "    got: $dir"
    fi

    if [[ -f "$repo/.wukong-code/sdd/.gitignore" && "$(cat "$repo/.wukong-code/sdd/.gitignore")" == "*" ]]; then
        pass "self-ignoring .gitignore created with '*'"
    else
        fail "self-ignoring .gitignore created with '*'"
    fi

    printf 'x\n' > "$repo/.wukong-code/sdd/artifact.md"
    local status
...
PLAN
    ...
    case "$brief_path" in
        "$repo/.wukong-code/sdd/"*) pass "task-brief writes its brief under the workspace" ;;
        *)
    ...
    case "$rp_path" in
        "$repo/.wukong-code/sdd/"*) pass "review-package writes its diff under the workspace" ;;
        *)
    ...
    if [[ "$wt_dir" == "$wt_root/.wukong-code/sdd" && "$wt_dir" != "$dir" ]]; then
    ...
    printf 'y\n' > "$wt/.wukong-code/sdd/artifact.md"
    local wt_status
