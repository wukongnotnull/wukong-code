#!/usr/bin/env bash
# Test: Plugin Loading
# Verifies that the wukong-code plugin loads correctly in OpenCode
set -euo pipefail
...
plugin_link="$OPENCODE_CONFIG_DIR/plugins/wukong-code.js"
...
skill_count=$(find "$WUKONG_CODE_SKILLS_DIR" -name "SKILL.md" | wc -l)
if [ "$skill_count" -gt 0 ]; then
    echo "  [PASS] Found $skill_count skills"
else
    echo "  [FAIL] No skills found in $WUKONG_CODE_SKILLS_DIR"
    exit 1
fi

# Test 3: Check using-wukong-code skill exists (critical for bootstrap)
echo "Test 3: Checking using-wukong-code skill (required for bootstrap)..."
if [ -f "$WUKONG_CODE_SKILLS_DIR/using-wukong-code/SKILL.md" ]; then
    echo "  [PASS] using-wukong-code skill exists"
else
    echo "  [FAIL] using-wukong-code skill not found (required for bootstrap)"
    exit 1
fi
...
if grep -q 'configDir}/skills/wukong-code/' "$WUKONG_CODE_PLUGIN_FILE"; then
