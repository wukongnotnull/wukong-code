...
# 1. Create in wukong-code location (lowest priority)
mkdir -p "$WUKONG_CODE_SKILLS_DIR/priority-test"
cat > "$WUKONG_CODE_SKILLS_DIR/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Wukong Code version of priority test skill
---
# Priority Test Skill (Wukong Code Version)

This is the WUKONG_CODE version of the priority test skill.

PRIORITY_MARKER_WUKONG_CODE_VERSION
EOF
...
if [ -f "$WUKONG_CODE_SKILLS_DIR/priority-test/SKILL.md" ]; then
    echo "  [PASS] Wukong Code version exists"
else
    echo "  [FAIL] Wukong Code version missing"
    exit 1
fi
...
# Test 2: Document personal vs bundled wukong-code priority
echo ""
echo "Test 2: Documenting personal vs wukong-code priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled wukong-code version instead of personal version"

# Test 3: Document project vs bundled wukong-code priority
echo ""
echo "Test 3: Documenting project vs personal/wukong-code priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled wukong-code version instead of project version"

# Test 4: Test a non-colliding bundled wukong-code skill is still available
echo ""
echo "Test 4: Testing non-colliding wukong-code skill remains available..."

mkdir -p "$WUKONG_CODE_SKILLS_DIR/wukong-code-only-test"
cat > "$WUKONG_CODE_SKILLS_DIR/wukong-code-only-test/SKILL.md" <<'EOF'
---
name: wukong-code-only-test
description: Wukong Code-only priority test skill
---
# Wukong Code Only Test Skill

PRIORITY_MARKER_WUKONG_CODE_ONLY_VERSION
EOF

run_opencode output "$TEST_HOME/test-project" "Call the skill tool with name \"wukong-code-only-test\". Show the exact content including any PRIORITY_MARKER text."
assert_contains "$output" "PRIORITY_MARKER_WUKONG_CODE_ONLY_VERSION" "Non-colliding wukong-code skill is still registered"
