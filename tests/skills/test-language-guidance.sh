...
if [[ -f "$skill" ]]; then
  ...
  assert_contains "$skill" "Inspecting fixture tests first is not a reason to delay testing.md. After TDD is selected, the first Read must be testing.md; do not Glob, Grep, or Read fixture or src in the same turn; any fixture or src Glob, Grep, or Read before that Read completes is a failure."
...
assert_contains skills/language-guidance/references/typescript/testing.md \
  "After TDD is selected, the first Read must be this file; do not Glob, Grep, or Read fixture or src in the same turn; starting process-all.test.js, process-all.test.ts, src/process-all.js, src/process-all.ts, or any fixture or src Glob, Grep, or Read before this Read completes is a failure."
...
assert_contains skills/test-driven-development/SKILL.md "For source work in a supported language, load language-guidance after selecting TDD. After TDD is selected, the first Read must be testing.md; do not issue any other Read, Glob, or Grep of fixture or src in the same turn; any fixture or src Glob, Grep, or Read, npm test, plan, or no-edit conclusion before that Read completes is a failure."
...
assert_contains skills/verification-before-completion/SKILL.md "The first verification command must be the repository test script; running node --check first, then reporting npm test skipped, is a verification failure."
