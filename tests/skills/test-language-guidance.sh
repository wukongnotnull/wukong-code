...
if [[ -f "$skill" ]]; then
  ...
  assert_contains "$skill" 'A Glob of skills/language-guidance or its references pack, including **/skills/language-guidance/** or Glob **/* whose target directory is skills/language-guidance, or a Read of typescript/implementation.md or rust/implementation.md is a failure when the prompt names TypeScript and Rust or the selected language is javascript; symmetrically, do not Read javascript/implementation.md or rust/implementation.md when the selected language is typescript, including to confirm they should not be loaded.'
...
assert_contains skills/test-driven-development/SKILL.md 'For source work in a supported language, load language-guidance after selecting TDD. After TDD SKILL.md is Read, the next file Read must be the selected language testing.md; Read of fixture src or tests in between is FAIL. After TDD or testing applies, a same-turn Glob **/* of javascript-basic, typescript-basic, or the fixture workspace after TDD SKILL.md, including while locating language-guidance via pack Glob, is forbidden until language-guidance testing.md Read completes, including after TDD SKILL.md or language-guidance SKILL.md were already Read; a workspace-root **/* that lists src or tests is a TDD failure; if TDD or testing will be used at all, fixture Reads cannot precede testing.md and late switch after exploring src (including during brainstorming) is still FAIL; any fixture or src Glob, Grep, Read, **/* workspace listing, npm test, plan, or no-edit conclusion before that Read completes is a failure.'
...
assert_contains README.md "| TypeScript | Experimental | ✓ | ✓ | ✓ | ✓ | ✓ | [Cursor recapture-15 draft](docs/wukong-code/evals/2026-09-01-typescript-language-guidance-cursor-recapture-15.md) — publication eval pending |"
assert_contains README.md "| JavaScript | Experimental | ✓ | ✓ | ✓ | ✓ | ✓ | [Cursor recapture-15 draft](docs/wukong-code/evals/2026-09-01-javascript-language-guidance-cursor-recapture-15.md) — publication eval pending |"
assert_contains README.md "publication eval pending"
