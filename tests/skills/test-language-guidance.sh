...
if [[ -f "$skill" ]]; then
  ...
  assert_contains "$skill" "Inspecting fixture tests first is not a reason to delay testing.md. After TDD is selected, the first Read must be testing.md; any fixture or src Read before that Read completes is a failure."
...
assert_contains skills/language-guidance/references/typescript/testing.md \
  "After TDD is selected, the first Read must be this file; starting process-all.test.js, process-all.test.ts, src/process-all.js, src/process-all.ts, or any fixture or src Read before this Read completes is a failure."
...
assert_contains skills/test-driven-development/SKILL.md "For source work in a supported language, load language-guidance after selecting TDD. After TDD is selected, the first Read must be testing.md; any fixture or src Read, npm test, plan, or no-edit conclusion before that Read completes is a failure."
