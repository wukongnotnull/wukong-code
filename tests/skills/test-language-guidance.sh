...
if [[ -f "$skill" ]]; then
  ...
  assert_contains "$skill" "Inspecting fixture tests first is not a reason to delay testing.md. Do not issue any other Read in the same turn as testing.md; wait until that Read has completed."
...
assert_contains skills/language-guidance/references/typescript/testing.md \
  "Do not issue any other Read in the same turn as this file; wait until this Read has completed. Starting process-all.test.js, process-all.test.ts, or any fixture test Read in that same turn is a failure."
