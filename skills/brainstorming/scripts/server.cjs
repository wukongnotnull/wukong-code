...
const WUKONG_CODE_VERSION = readWukongCodeVersion();
...
function readWukongCodeVersion() {
...
function brandMarkup() {
  // Local text branding only — no remote logo / telemetry image fetch.
  const version = escapeHtmlText(WUKONG_CODE_VERSION);
  const text = 'Wukong Code v' + version;

  return '<div class="brand"><a href="https://github.com/wukongnotnull/wukong-code"><span class="brand-copy">' + text + '</span></a></div>';
}
