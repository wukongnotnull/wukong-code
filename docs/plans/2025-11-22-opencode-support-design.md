...
Add full wukong-code support for OpenCode.ai using a native OpenCode plugin architecture that shares core functionality with the existing Codex implementation.

## Background

OpenCode.ai is a coding agent similar to Claude Code and Codex. Previous attempts to port wukong-code to OpenCode (PR #93, PR #116) used file-copying approaches. This design takes a different approach: building a native OpenCode plugin using their JavaScript/TypeScript plugin system while sharing code with the Codex implementation.
...
2. **Platform-Specific Wrappers**
   - Codex: CLI script (`.codex/wukong-code-codex`)
   - OpenCode: Plugin module (`.opencode/plugin/wukong-code.js`)

3. **Skill Directories**
   - Core: `~/.config/opencode/wukong-code/skills/` (or installed location)
   - Personal: `~/.config/opencode/skills/` (shadows core skills)

### Code Reuse Strategy

Extract common functionality from `.codex/wukong-code-codex` into shared module:
...
```javascript
{
  ...
  schema: z.object({
    skill_name: z.string().describe('Name of skill (e.g., "wukong-code:brainstorming")')
  }),
...
1. **Inject using-wukong-code content**
   - Full content of the using-wukong-code skill
...
```javascript
// .opencode/plugin/wukong-code.js
...
export const WukongCodePlugin = async ({ client, directory, $ }) => {
  const wukong-codeDir = path.join(process.env.HOME, '.config/opencode/wukong-code');
  const personalDir = path.join(process.env.HOME, '.config/opencode/skills');

  return {
    'session.started': async () => {
      const usingWukong Code = await readSkill('using-wukong-code');
      const skillsList = await findAllSkills();
      const toolMapping = getToolMappingInstructions();

      return {
        context: `${usingWukong Code}\n\n${skillsList}\n\n${toolMapping}`
      };
    },
...
├── .codex/
│   ├── wukong-code-codex        # UPDATED: Use skills-core
│   ├── wukong-code-bootstrap.md
│   └── INSTALL.md
├── .opencode/
│   ├── plugin/
│   │   └── wukong-code.js       # NEW: OpenCode plugin
...
1. Create `lib/skills-core.js`
   - Extract frontmatter parsing from `.codex/wukong-code-codex`
...
2. Update `.codex/wukong-code-codex` to use shared core
...
1. Create `.opencode/plugin/wukong-code.js`
