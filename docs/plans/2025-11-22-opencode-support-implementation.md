...
> **For agentic workers:** REQUIRED SUB-SKILL: Use wukong-code:executing-plans to implement this plan task-by-task.

**Goal:** Add full wukong-code support for OpenCode.ai with a native JavaScript plugin that shares core functionality with the existing Codex implementation.
...
- Reference: `.codex/wukong-code-codex` (lines 40-74)
...
- Reference: `.codex/wukong-code-codex` (lines 97-136)
 ...
 * @param {string} sourceType - 'personal' or 'wukong-code' for namespacing
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/wukong-code-codex` (lines 212-280)
 ...
 * (personal skills override wukong-code skills).
 *
 * @param {string} skillName - Name like "wukong-code:brainstorming" or "my-skill"
 * @param {string} wukong-codeDir - Path to wukong-code skills directory
 * @param {string} personalDir - Path to personal skills directory
 * @returns {{skillFile: string, sourceType: string, skillPath: string} | null}
 */
function resolveSkillPath(skillName, wukong-codeDir, personalDir) {
    // Strip wukong-code: prefix if present
    const forceWukong Code = skillName.startsWith('wukong-code:');
    const actualSkillName = forceWukong Code ? skillName.replace(/^wukong-code:/, '') : skillName;

    // Try personal skills first (unless explicitly wukong-code:)
    if (!forceWukong Code && personalDir) {
    ...
    // Try wukong-code skills
    if (wukong-codeDir) {
        const wukong-codePath = path.join(wukong-codeDir, actualSkillName);
        const wukong-codeSkillFile = path.join(wukong-codePath, 'SKILL.md');
        if (fs.existsSync(wukong-codeSkillFile)) {
            return {
                skillFile: wukong-codeSkillFile,
                sourceType: 'wukong-code',
                skillPath: actualSkillName
            };
        }
    }
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/wukong-code-codex` (lines 16-38)
...
**Files:**
- Modify: `.codex/wukong-code-codex` (add import at top)
...
Run: `node -c .codex/wukong-code-codex`
Expected: No output

**Step 3: Commit**

```bash
git add .codex/wukong-code-codex
...
**Files:**
- Modify: `.codex/wukong-code-codex` (lines 40-74)
...
Run: `.codex/wukong-code-codex find-skills | head -20`
...
**Files:**
- Modify: `.codex/wukong-code-codex` (lines 97-136, approximately)
...
**Files:**
- Modify: `.codex/wukong-code-codex` (lines 16-38, approximately)
...
Run: `.codex/wukong-code-codex bootstrap | head -50`
...
**Files:**
- Create: `.opencode/plugin/wukong-code.js`
...
/**
 * Wukong Code plugin for OpenCode.ai
 *
...
const wukongCodeSkillsDir = path.join(homeDir, '.config/opencode/wukong-code/skills');
const personalSkillsDir = path.join(homeDir, '.config/opencode/skills');

/**
 * OpenCode plugin entry point
 */
export const WukongCodePlugin = async ({ project, client, $, directory, worktree }) => {
  return {
...
Run: `ls -l .opencode/plugin/wukong-code.js`
Expected: File exists

**Step 4: Commit**

```bash
git add .opencode/plugin/wukong-code.js
...
**Files:**
- Modify: `.opencode/plugin/wukong-code.js`
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "wukong-code:brainstorming" or "my-custom-skill")')
        }),
        execute: async ({ skill_name }) => {
          // Resolve skill path (handles shadowing: personal > wukong-code)
...
Run: `node -c .opencode/plugin/wukong-code.js`
Expected: No output
...
```javascript
      {
        name: 'find_skills',
        description: 'List all available skills in the wukong-code and personal skill libraries.',
        schema: z.object({}),
        execute: async () => {
          // Find skills in both directories
          const wukong-codeSkills = skillsCore.findSkillsInDir(
            wukongCodeSkillsDir,
            'wukong-code',
            3
          );
          ...
          const allSkills = [...personalSkills, ...wukong-codeSkills];

          if (allSkills.length === 0) {
            return 'No skills found. Install wukong-code skills to ~/.config/opencode/wukong-code/skills/';
          }

          let output = 'Available skills:\n\n';

          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'wukong-code:';
...
```javascript
    'session.started': async () => {
      // Read using-wukong-code skill content
      const usingWukong CodePath = skillsCore.resolveSkillPath(
        'using-wukong-code',
        wukongCodeSkillsDir,
        personalSkillsDir
      );

      let usingWukong CodeContent = '';
      if (usingWukong CodePath) {
        const fullContent = fs.readFileSync(usingWukong CodePath.skillFile, 'utf8');
        // Strip frontmatter
        const lines = fullContent.split('\n');
        let inFrontmatter = false;
        let frontmatterEnded = false;
        ...
        usingWukong CodeContent = contentLines.join('\n').trim();
      }
...
**Skills naming:**
- Wukong Code skills: \`wukong-code:skill-name\` (from ~/.config/opencode/wukong-code/skills/)
...
`;
      ...
      const hasUpdates = skillsCore.checkForUpdates(
        path.join(homeDir, '.config/opencode/wukong-code')
      );

      const updateNotice = hasUpdates ?
        '\n\n⚠️ **Updates available!** Run `cd ~/.config/opencode/wukong-code && git pull` to update wukong-code.' :
        '';
...
**Below is the full content of your 'wukong-code:using-wukong-code' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**

${usingWukong CodeContent}
...
```markdown
# Installing Wukong Code for OpenCode
...
### 1. Install Wukong Code Skills

```bash
# Clone wukong-code skills to OpenCode config directory
mkdir -p ~/.config/opencode/wukong-code
git clone https://github.com/wukongnotnull/wukong-code.git ~/.config/opencode/wukong-code
```

### 2. Install the Plugin

The plugin is included in the wukong-code repository you just cloned.

OpenCode will automatically discover it from:
- `~/.config/opencode/wukong-code/.opencode/plugin/wukong-code.js`
...
ln -s ~/.config/opencode/wukong-code/.opencode/plugin/wukong-code.js .opencode/plugin/wukong-code.js
```
...
```
use use_skill tool with skill_name: "wukong-code:brainstorming"
```
...
Personal skills override wukong-code skills with the same name.

## Updating

```bash
cd ~/.config/opencode/wukong-code
git pull
```
...
1. Check plugin file exists: `ls ~/.config/opencode/wukong-code/.opencode/plugin/wukong-code.js`
2. Check OpenCode logs for errors
3. Verify Node.js is installed: `node --version`

### Skills not found

1. Verify skills directory exists: `ls ~/.config/opencode/wukong-code/skills`
...
- Documentation: https://github.com/wukongnotnull/wukong-code
```
...
Wukong Code works with [OpenCode.ai](https://opencode.ai) through a native JavaScript plugin.
...
Run: `.codex/wukong-code-codex use-skill wukong-code:brainstorming | head -20`
Expected: Shows brainstorming skill content

**Step 3: Test bootstrap command**

Run: `.codex/wukong-code-codex bootstrap | head -30`
...
ls -l .opencode/plugin/wukong-code.js
...
- Total commits made
- Files created: `lib/skills-core.js`, `.opencode/plugin/wukong-code.js`, `.opencode/INSTALL.md`
- Files modified: `.codex/wukong-code-codex`, `README.md`, `RELEASE-NOTES.md`
...
- [ ] `.codex/wukong-code-codex` refactored to use shared core
- [ ] Codex commands still work (find-skills, use-skill, bootstrap)
- [ ] `.opencode/plugin/wukong-code.js` created with tools and hooks
