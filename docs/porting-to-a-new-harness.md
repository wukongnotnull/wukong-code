# Porting Wukong Code to a New Harness

This guide explains how to add support for a new harness — an IDE, CLI, or
agent runner that isn't Claude Code — so that Wukong Code skills auto-trigger
...
## Part 1 — How Wukong Code works across harnesses

Wukong Code is the same content everywhere. What changes per harness is the thin
...
A harness can support Wukong Code only if it can do all of the following. Check
...
model to describe its wukong-code. If the bootstrap injected, it knows it has
them. (OpenCode's install doc uses `opencode run --print-logs "hello" 2>&1 |
grep -i wukong-code` for the same goal via a different mechanism — log-grep
...
`using-wukong-code` skill's own description at session start). If more than one
...
`using-wukong-code/SKILL.md` and prints a JSON object whose **field name and
...
- Reference: `.opencode/plugins/wukong-code.js` (JavaScript) and
  `.pi/extensions/wukong-code.ts` (TypeScript). pi is the closest reference for
...
- **Shape B:** the module the harness loads (e.g. `.<harness>/plugins/*.js`) plus
  ...
  - **Build/dependency check.** Decide how the harness loads your module:
    ...
    step? Wukong Code is zero-runtime-dependency. pi's `import type
...
`using-wukong-code` skill content (wrapped in `<EXTREMELY_IMPORTANT>` tags) plus
...
field/nesting, matcher strings — are the harness's contract, not Wukong Code's,
...
- **Compaction.** If the harness compacts/summarizes history, re-inject
  ...
  cannot safely skip compact re-inject: SessionStart stdin has no documented
...
`using-wukong-code/SKILL.md` tells the model to *never read skill files manually
...
2. **Native skill *discovery* but no `Skill` tool** (pi, Antigravity): the harness
   ...
   **Fallback — the surfaced skill index.** If there's no context-file field but
   ...
   is the index, and `using-wukong-code`'s own surfaced description can be what
   ...
   - **It bootstraps *triggering*, not the *tool mapping*.** An injector prepends
     `<harness>-tools.md` alongside `using-wukong-code` every session. Here nothing
...
3. **No skill system at all:** there is nothing to register, and the *only*
   ...
   what it can't find: `using-wukong-code/SKILL.md` does **not** enumerate the
...
# 3. Smoke check: does the model know it has wukong-code?
...
tmux send-keys -t port-test 'What are your wukong-code?'; sleep 0.4; tmux send-keys -t port-test Enter
sleep 5
...
If the smoke check shows the model *doesn't* know it has wukong-code, the
...
- Fill in the PR template's **"New harness support"** section and paste the
  ...
  be closed.
- Wukong Code is a zero-dependency plugin. Don't add a third-party runtime
...
- **Opt-in isn't a port.** If your human partner has to do anything per session
  to get Wukong Code, the acceptance test fails. Re-read Part 2.
